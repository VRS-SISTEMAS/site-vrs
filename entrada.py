...
# MÓDULO: entrada.py (Versão High-End - Sem Rolagem)
# =================================================================
...
<style>
    ...
    .vrs-header {
        ...
        top: -75px;
        left: 0;
        font-size: 18px; /* Aumentado conforme solicitado */
        color: #00c853;
        font-weight: bold;
        letter-spacing: 3px;
    ...
    .container-vantagens {
        ...
        border-radius: 20px;
        padding: 20px 40px; /* Reduzido o padding vertical */
        border: 1px solid #222;
        margin-bottom: 20px; /* Reduzido para caber tudo */
    ...
    .card-vrs {
        ...
        height: 310px; /* Altura reduzida para evitar rolagem */
    }
...
def exibir_home():
    ...
    botoes.barra_navegacao(pagina_anterior=None, pagina_proxima='checkout')
...
if st.session_state['pagina_ativa'] == 'home':
    ...
    try:
        with open("checkout.py", encoding="utf-8") as f:
            exec(f.read())
    except Exception as e:
        st.error(f"Erro: {e}")
elif st.session_state['pagina_ativa'] == 'sucesso':
    try:
        with open("sucesso.py", encoding="utf-8") as f:
            exec(f.read())
    except Exception as e:
        st.error(f"Erro: {e}")